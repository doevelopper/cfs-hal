  ...
  struct UartConfig 
  {
    ...
    */ 
    enum class UartHardwareFlowControl : std::uint8_t
    {
      ...
      AUART_HARDWARE_FLOW_CONTROL_XONXOFF /** Software flow control (XON/XOFF). */
      AUART_HARDWARE_FLOW_CONTROL_RTSCTS_XONXOFF, /** Hardware flow control (RTS/CTS). */
    };
    /*!
    ...
    */ 
    ...
    public:
        ...
        Uart(const std::string& path, std::uint32_t baudrate, UartDataBits databits, UartParity parity, UartStopBits stopbits, UartHardwareFlowControl handshake);
        Uart(const std::string& path, std::uint32_t baudrate, UartDataBits databits, UartParity parity, UartStopBits stopbits);
